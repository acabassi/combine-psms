...
### Load results
load("results/binary-outcome/settingA_outcomeguided.RData")
ari_og <- ari_all; rm(ari_all)
weights_og <- weights; rm(weights)
load("results/binary-outcome/settingA_unsupervised.RData")
ari_un <- ari_all; rm(ari_all)
weights_un <- weights; rm(weights)
load("results/binary-outcome/summarising_PSMs.RData")
...
save(ari, file = "results/binary-outcome/settingA_ari.RData")
ari.m <- melt(ari)             
head(ari.m)
...
ggsave(paste0("figures/binary-outcome/ari-a-outcomeguided.jpg"),
...
save(weights, file = "results/binary-outcome/settingA_weights.RData")
...
ggplot(data = weights.m, aes(x=Dataset, y=Weight, fill = Dataset)) + 
  ...
  scale_fill_manual(values=ari_palette[1:4])
ggsave(paste0("figures/binary-outcome/weights-a-outcomeguided.jpg"),
