...
for(experiment in 1:n_experiments){
  ...
  for(w in w_values){
    ...
    for(n_covariates in possible_n_covariates){
      ...
      cl_draws <- minVI(psm, cls.draw = t(Z), method = "draws")
      
      # Compute clusterings
      cl_avg_6 <- minVI(psm, method = "avg", max.k = 6)
      cl_avg_20 <- minVI(psm, method = "avg", max.k = 20)
      cl_comp_6 <- minVI(psm, method = "comp", max.k = 6)
      cl_comp_20 <- minVI(psm, method = "comp", max.k = 20)
...
save(ari_avg_6, ari_avg_20, ari_comp_6, ari_comp_20, ari_draws,
     file = "results/summarising_psms_min_vi.RData")
