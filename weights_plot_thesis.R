...
load("results/binary-outcome/settingA_weights.RData")
weights_a <- weights; rm(weights)

load("results/binary-outcome/settingB_weights_10_var_sel.RData")
weights_b <- weights; rm(weights)

load("results/binary-outcome/settingC_weights.RData")
weights_c <- weights; rm(weights)

load("results/binary-outcome/settingD_weights.RData")
...
weights.m$Subset <- as.character(weights.m$Subset)
weights.m$Subset <- replace(weights.m$Subset, weights.m$Subset=="1+2+3", "0+1+2")
weights.m$Subset <- replace(weights.m$Subset, weights.m$Subset=="1+2+4", "0+1+3")
weights.m$Subset <- replace(weights.m$Subset, weights.m$Subset=="1+3+4", "0+2+3")
weights.m$Subset <- replace(weights.m$Subset, weights.m$Subset=="2+3+4", "1+2+3")
weights.m$Subset <- as.factor(weights.m$Subset)
...
ggsave(paste0("figures/binary-outcome/weights-outcomeguided-binary-outcome.pdf"),
       device = "pdf", width = 14, height = 17.5,
       units = "cm")
