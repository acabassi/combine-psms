...
for experiment = 1:n_experiments
    for w = 1:3
        load(strcat('~/OneDrive - University of Cambridge, MRC Biostatistics Unit/PHD-PROJECTS/klic-psm-code/kernels-matlab/experiment', int2str(experiment), '_w', int2str(w*2), '_ncov10_chain1_psm.mat'))
        all_psms(:,:,experiment,w) = psm;
    end
    w=4;
    load(strcat('~/OneDrive - University of Cambridge, MRC Biostatistics Unit/PHD-PROJECTS/klic-psm-code/kernels-matlab/experiment', int2str(experiment), '_w', int2str(w*2), '_ncov10_chain1_psm_permuted.mat'))
    all_psms(:,:,experiment,w) = psm;
end
%% Response
...
save('results/settingD_outcomeguided_weights.mat', 'all_betas');
