...
### Load results
load(paste0("results/binary-outcome/settingD_unsupervised.RData"))
ari_un <- ari_all; rm(ari_all)
weights_un <- weights; rm(weights)
load(paste0("results/binary-outcome/settingD_outcomeguided.RData"))
ari_og <- ari_all; rm(ari_all)
weights_og <- weights; rm(weights)
load(paste0("results/binary-outcome/settingD_unsupervised_one.RData"))
...
save(ari, file = "results/binary-outcome/settingD_ari.RData")
...
ggsave(paste0("figures/binary-outcome/ari-d-outcomeguided.jpg"),
...
dimnames(weights)[4][[1]] <- c("Unsupervised integration",
                               "Outcome-guided integration")
save(weights, file = "results/binary-outcome/settingD_weights.RData")
...
ggplot(data = weights.m, aes(x=Dataset, y=Weight, fill = Dataset)) + 
  ...
  scale_fill_manual(values=ari_palette)
ggsave(paste0("figures/binary-outcome/weights-d-outcomeguided.jpg"),
