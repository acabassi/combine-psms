...
%% Load output of simpleMKL

options.algo = 'oneagainstall';

if options.algo == 'oneagainstall'
    load('~/Documents/combining-PSMs/weights_simpleMKL_multiclassOAA.mat');
elseif options.algo == 'oneagainstone'
    load('~/Documents/combining-PSMs/weights_simpleMKL_multiclassOAO.mat');
end
...
subsets_w = [2,3,4; 2,3,5; 2,4,5; 3,4,5]; % subsets of simulated datasets

for j = 1:4
        indices = subsets_w(j,:);
    for i = 1:100
        psm_ij = w(i,j,1)*reshape(psm(:,:,i,indices(1)),[300,300])+...
                 w(i,j,2)*reshape(psm(:,:,i,indices(2)),[300,300])+...
                 w(i,j,3)*reshape(psm(:,:,i,indices(3)),[300,300]);
...
if options.algo == 'oneagainstall'
save('~/Documents/combining-PSMs/densityMatrix_KmeansMKL_OAA.mat','densityMatrix')
elseif options.algo == 'oneagainstone'
    save('~/Documents/combining-PSMs/densityMatrix_KmeansMKL_OAO.mat', 'densityMatrix')
end
