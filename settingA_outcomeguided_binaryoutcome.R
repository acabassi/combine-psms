...
weights <- readMat("results/binary-outcome/settingA_outcomeguided_weights.mat")$all.betas

for(experiment in 1:n_experiments){
  cat("Experiment", experiment, " ")
  weighted_psm <- matrix(0, N, N)
...
save(ari_all, weights, file = "results/binary-outcome/settingA_outcomeguided.RData")
