...
weights <- readMat("results/binary-outcome/settingC_outcomeguided_weights.mat")$all.betas
  ...
  for(subset in 1:n_subsets){
    ...
    for(count in 1:n_psms_per_subset){
      ...
      if(w!=0.8){
        ...
        load(paste0("premium/binary-outcome/experiment", experiment, "_w", w*10, "_ncov", P,
                  "_chain", 1, "_psm_exclude_y_permuted.RData"))
      }
...
save(ari_all, weights, file = "results/binary-outcome/settingC_outcomeguided.RData")
